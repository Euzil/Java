...
public class ConnJavaDB {
	...
	static { // 通过静态方法加载数据库驱动，并且在数据库不存在的情况下创建数据库
		try {
			...
			System.out.println("JavaDB数据库驱动加载成功。");
			...
			if (!albumF.exists()) { // 判断数据库文件是否存在
				...
				System.out.println("成功与JavaDB数据库建立连接。");
	...
	public static void main(String[] args) {
		ConnJavaDB connJavaDB = new ConnJavaDB();
		connJavaDB.getConnection();
	}
}
