...
public class BookSale {// 创建图书销售量类
	...
	public int getBookSale() {// 获得图书销售量的方法
		...
		try {
			Statement statement = conn.createStatement();// 使用数据库连接对象创建一个Statement对象
			String sql = "select sum(bookCount) from tb_book where month(bookDate) = 6";// 查询编程词典销量表中6月的总销量
			ResultSet set = statement.executeQuery(sql);// 执行查询语句返回查询结果集
