...
class TurnColor extends Thread { // 创建“改变颜色”类继承线程类
	...
	public void run() { // 线程执行的任务
		while (true) { // 线程始终处于被启用状态
			...
			lblImage.setIcon(new ImageIcon(TraficLights.class.getResource("Yellow.png"))); // 黄灯
			try {
				Thread.sleep(2000); // 线程休眠2秒
			} catch (InterruptedException e) {
				e.printStackTrace();
			}
			lblImage.setIcon(new ImageIcon(TraficLights.class.getResource("Red.png"))); // 红灯
			try {
				Thread.sleep(8000); // 线程休眠8秒
			} catch (InterruptedException e) {
				e.printStackTrace();
			}
			lblImage.setIcon(new ImageIcon(TraficLights.class.getResource("Green.png"))); // 绿灯
		}
	}
}